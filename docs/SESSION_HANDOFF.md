...
- The App room uses the project-owner supplied bright monochrome Pantip Plaza / technology-mall photograph as a local 1672×941 EXIF-free RGB WebP wallpaper at `assets/app-room/owner-supplied-pantip-plaza-wallpaper.webp`. Navigation, header, search/tools, footer, and the full App shelf wall use App-scoped translucent retail/glass surfaces while preserving the 1990s software-floor language, LED signage, Windows 95-style search panel, metal shelf edges, and all six diskette cards. Public-safe provenance is recorded in `docs/reports/APP_ROOM_IMAGE_ATTRIBUTION.md`.
