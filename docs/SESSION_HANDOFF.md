...
- `Podcast_Visual_Storyboard_AI_Prompting_Manual.html` is the newest Reading guide: built from the owner-provided public Markdown into the established 10-view Interactive Reference Manual pattern with sidebar/topbar navigation, hash routing, search, theme/font state, copy controls, print CSS, and no reader-facing `ค่ะ`. The owner-supplied final cover is used as the active shelf/catalog cover at `assets/covers/custom/podcast-visual-storyboard-ai-prompting.webp`, normalized only by resizing/padding to 600×900 RGB WebP and stripping metadata while preserving the visible design.
