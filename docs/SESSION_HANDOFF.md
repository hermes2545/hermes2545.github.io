...
- 31 standalone HTML guides indexed from `data/books.json`.
...
- `Podcast_Visual_Storyboard_AI_Prompting_Manual.html` is the newest Reading guide: replaced on 2026-09-02 with the owner-supplied final HTML, preserved byte-for-byte at SHA-256 `145ffc4cf7d08a74b78ebb6ab8d11b41931a00533b49d04699c6ad5f066fc4fc`. The replacement manual is an Interactive Reference Manual v2.0 with 1 overview view, 22 chapter views, 23 navigation buttons, search, theme/font state, copy controls, mobile drawer/search, print CSS, and no reader-facing `ค่ะ`. The owner-supplied final cover remains the active shelf/catalog cover at `assets/covers/custom/podcast-visual-storyboard-ai-prompting.webp`, normalized only by resizing/padding to 600×900 RGB WebP and stripping metadata while preserving the visible design.
- `Human_AI_Communication_Framework_Interactive_Reference_Manual.html` is the previous Reading guide: the owner-supplied bilingual HTML is published byte-for-byte at SHA-256 `95de03e50046b439a5a7deb2793359cf4da887001fe893d2e1be53fec5dc1fb9`, with 1 home page, 15 content pages, 16 navigation buttons, `localStorage`, image viewer controls, theme/language/font controls, and no reader-facing `ค่ะ`. Its owner-supplied cover is normalized only for shelf delivery to an EXIF-free 600×900 RGB WebP at `assets/covers/custom/human-ai-communication-framework.webp`, SHA-256 `90ab0e70d9b6df68ecf8011b579fa4647cf0859ce664fba414a373c0cb079dbf`.
- `GrokRouter_Interactive_Reference_Manual_TH.html` is the previous Reading guide: the owner-supplied TH3 HTML is published as a 22-view/22-nav Interactive Reference Manual at SHA-256 `12ad8451b29c3b83003a3c037f76b9bdca4f2dd2fdb2377e5a3661c64510e64f`, with a robust document scroll fix, `localStorage`, `navigator.clipboard`, print CSS, and no reader-facing `ค่ะ`. Its owner-supplied cover is normalized only for the shelf to an EXIF-free 600×900 RGB WebP at `assets/covers/custom/grokrouter-interactive-reference-manual.webp`.
...
- Added Audio Shelf catalog entry at position 1 with `assets/audio-covers/AJ4cLr7A4Rc.jpg`, duration 1335 seconds, publish timestamp `2026-09-01T21:17:41+07:00`, and regenerated `audio-library.html` with 53 entries. Focused Audio tests passed and full gates passed with **114 tests**, all four generator `--check` commands, and `git diff --check` clean. Published in commit `681c1a9a7faf83568a93b794d167a7396c8eacc0`; `origin` and `backup` `main` both read back the same SHA, production `audio-library.html` and cover SHA-256 matched Local, and live production DOM showed 53 cards with the new item first, YouTube link, duration 22:15, zero horizontal overflow, and total count 53.
...
- Verification before the current-turn approved push passed: focused Human–AI and catalog tests, full `python -m unittest discover -s tests -v` with **113 tests**, all four generator `--check` commands, `git diff --check`, pre-share scan of intended files with no credential/private-path findings, and Playwright desktop/mobile screenshots for Reading shelf and manual with no visible layout break or horizontal overflow. Production read-back matched Local for `index.html`, the manual HTML, and the cover asset.
...
- Replaced the earlier staged HTML with the latest owner-supplied replacement file and locked the final SHA-256 to `12ad8451b29c3b83003a3c037f76b9bdca4f2dd2fdb2377e5a3661c64510e64f`. The manual has 22 pages, 22 navigation buttons, robust document scroll fix markers, `localStorage`, `navigator.clipboard`, print CSS, and no reader-facing `ค่ะ`.
