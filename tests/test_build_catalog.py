...
class HomepageBuildTests(unittest.TestCase):
    ...
    def test_reading_book_cover_uses_stable_css_3d_front_cover_peek(self):
        ...
        self.assertIn(".reading-page .book-cover-volume {\n  position: relative;\n  width: 100%;\n  aspect-ratio: 2 / 3;\n  perspective: 1100px;", stylesheet)
        ...
        self.assertIn("transform .42s cubic-bezier(.2,.8,.2,1)", stylesheet)
        self.assertIn("box-shadow .42s cubic-bezier(.2,.8,.2,1)", stylesheet)
        ...
        self.assertIn(".reading-page .book-card:focus-visible .book-cover", stylesheet)
        self.assertIn(".reading-page .book-cover-link:focus-visible .book-cover", stylesheet)
        self.assertNotIn("@media (hover: hover), (any-hover: hover) {\n  .reading-page .book-card:hover .book-cover", stylesheet)

        script = (ROOT / "assets" / "js" / "library.js").read_text(encoding="utf-8")
        self.assertIn("function bindReadingCoverHoverFallback", script)
        self.assertIn("card.classList.toggle(\"is-open\", open)", script)
        self.assertIn('card.addEventListener("pointerenter"', script)
        self.assertIn('card.addEventListener("mousemove"', script)
        self.assertIn('card.addEventListener("pointerleave"', script)
        ...
        self.assertIn('document.addEventListener("pointerover"', script)
        self.assertIn('event.target.closest(".book-card")', script)
        self.assertIn("elementFromPoint(event.clientX, event.clientY)", script)
        self.assertIn('target.closest(".book-card")', script)
        ...
        self.assertIn("transform-style: preserve-3d;", stylesheet)
        self.assertIn(".reading-page .book-cover-volume::before {\n  content: \"\";\n  position: absolute;\n  inset: 4px 3px 4px 15px;\n  z-index: 0;", stylesheet)
        self.assertIn("transform: translateX(5px);", stylesheet)
        self.assertIn("will-change: transform;", stylesheet)
    ...
    def test_coffee_theme_is_loaded_only_by_the_reading_collection(self):
        ...
        self.assertIn('href="assets/css/reading-library.css?v=reading-cover-hover-v5"', reading)
        self.assertIn('href="assets/css/reading-library.css?v=reading-cover-hover-v5"', self.html)
        self.assertIn('src="assets/js/library.js?v=reading-cover-hover-v5"', reading)
        self.assertIn('src="assets/js/library.js?v=reading-cover-hover-v5"', self.html)
