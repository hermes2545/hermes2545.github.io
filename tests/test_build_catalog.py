...
class HomepageBuildTests(unittest.TestCase):
    ...
    def test_reading_book_cover_uses_stable_css_3d_front_cover_peek(self):
        ...
        self.assertNotIn(".reading-page .book-card.is-open .book-cover", stylesheet)
        ...
        self.assertNotIn("function bindReadingCoverHoverFallback", script)
        self.assertNotIn("is-open", script)
        self.assertNotIn('document.addEventListener("mousemove"', script)
        ...
        self.assertIn(".reading-page .book-cover {\n  position: relative;\n  z-index: 1;\n  width: 100%;\n  height: auto;\n  display: block;\n  border-radius: 1px 3px 3px 1px;", stylesheet)
        cover_rule = stylesheet.split(".reading-page .book-cover {", 1)[1].split("}\n", 1)[0]
        self.assertNotIn("position: absolute;", cover_rule)
        self.assertNotIn("inset: 0;", cover_rule)
        self.assertNotIn("object-fit: cover;", cover_rule)
        ...
        self.assertNotIn(".42s cubic-bezier(.2, .8, .2, 1)", stylesheet)
        self.assertNotIn("border-radius: 18px 22px 22px 18px;", stylesheet)
        self.assertNotIn("border-radius: 14px 20px 20px 14px;", stylesheet)
    ...
    def test_coffee_theme_is_loaded_only_by_the_reading_collection(self):
        ...
        self.assertIn('href="assets/css/reading-library.css?v=reading-cover-hover-v11"', reading)
        self.assertIn('href="assets/css/reading-library.css?v=reading-cover-hover-v11"', self.html)
        self.assertIn('src="assets/js/library.js?v=reading-cover-hover-v11"', reading)
        self.assertIn('src="assets/js/library.js?v=reading-cover-hover-v11"', self.html)
