...
class AudioLibraryTests(unittest.TestCase):
    ...
    def test_playlist_catalog_is_complete_and_unique(self):
        ...
        self.assertEqual(len({item["video_id"] for item in self.items}), 54)
        self.assertEqual(len({item["youtube_url"] for item in self.items}), 54)
    ...
    def test_catalog_is_sorted_newest_first_with_playlist_order_as_tiebreak(self):
        ...
        self.assertEqual(self.items[0]["video_id"], "sKC0mlraNPo")
    ...
    def test_latest_demand_collapse_storyboard_podcast_metadata(self):
        ...
        self.assertEqual(item["youtube_url"], "https://www.youtube.com/watch?v=sKC0mlraNPo")
    ...
    def test_page_renders_all_items_with_dates_durations_and_new_tabs(self):
        self.assertEqual(self.page.count('class="book-card audio-card"'), 54)
        self.assertEqual(self.page.count('target="_blank"'), 56)  # 54 items + playlist button + footer social link
        self.assertEqual(self.page.count('rel="noopener"'), 56)
        self.assertIn('class="footer-facebook-link"', self.page)
        self.assertEqual(self.page.count('publish on '), 54)
        self.assertEqual(self.page.count('class="audio-duration"'), 54)
    ...
    def test_audio_covers_show_full_four_by_three_thumbnail_over_play_panel(self):
        self.assertEqual(self.page.count('class="audio-thumbnail-frame"'), 54)
        self.assertEqual(self.page.count('class="audio-play-panel"'), 54)
        self.assertEqual(self.page.count('class="audio-play-button"'), 54)

    def test_audio_cards_are_ipods_without_audio_book_kickers(self):
        self.assertNotIn("AUDIO BOOK", self.page)
        self.assertEqual(self.page.count('class="book-cover-wrap audio-cover-wrap audio-ipod"'), 54)
        self.assertEqual(self.page.count('class="audio-click-wheel"'), 54)
        stylesheet = (ROOT / "assets" / "css" / "audio-library.css").read_text(encoding="utf-8")
    ...
    def test_duration_is_in_the_lower_panel_above_play(self):
        ...
        self.assertEqual(self.page.count('class="audio-play-label">PLAY'), 54)
